import { useState } from "react";
import Filters from "./Filters"
import FlatDisplay from "./FlatDisplay"

function Flats({filtered_flats, handleactiononroom , actiononroom , roomtype , setRoomType}) {
  
    console.log("filtered fats from the Flat.jsx is ", filtered_flats);
    return (
        <div className=" h-full md:w-7/12 py-1 w-full border  bg-stone-200 rounded-3xl overflow-y-auto custom-scrollbar mx-2 dark:bg-stone-800 border-transparent">
            <Filters handleactiononroom={handleactiononroom} actiononroom={actiononroom} roomtype={roomtype} setRoomType={setRoomType} />
            <h2 className="ml-3 text-xl font-semibold dark:text-stone-200">Best Options</h2>

            <FlatDisplay  filtered_flats={filtered_flats}/>
        </div>
    )
}

export default Flats
